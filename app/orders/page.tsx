...
export default function UploadOrderPage() {
  ...
  const [charliesTotal, setCharliesTotal] = useState(0);
  const [ogcTotal, setOgcTotal] = useState(0);
  ...
  return (
    <section className="container py-10 flex flex-col">
      <div className="text-lg fixed top-0 right-0 rounded-bl-lg bg-muted text-foreground py-4 px-8">
        <div className="flex gap-2">
          ...
          <span className="font-semibold">
            ${Math.abs(charliesTotal + ogcTotal).toFixed(2)}
          </span>
        </div>
        {charliesData && (
          <div className="flex gap-2">
            <span className="font-semibold">Charlie&apos;s Total:</span>
            <span className="">${Math.abs(charliesTotal).toFixed(2)}</span>
          </div>
        )}
        {OGCData && (
          <div className="flex gap-2">
            <span className="font-semibold">OGC Total:</span>
            <span className="">${Math.abs(ogcTotal).toFixed(2)}</span>
          </div>
        )}
      </div>
      ...
      <div className="container px-0">
        {!!charliesData && (
          <CharliesDataTable
            columns={charliesColumns}
            data={charliesData}
            setOrderTotal={setCharliesTotal}
          />
        )}
        {!!OGCData && (
          <OGCDataTable
